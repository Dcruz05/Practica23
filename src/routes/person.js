...
router.get('/deletePerson/:id', (req, res)=>{ // buscar por ID para poder mandarlo a bucar y borrar
    Person.findByIdAndDelete(req.params.id).then(()=>{res.redirect('/')}).catch((err)=>{res.json({message:err});})//si se borra correctamente nos redirige a index si no muestra un error en json
})
